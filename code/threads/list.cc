...
List::Remove()
{
return SortedRemove(NULL); // Same as SortedRemove, but ignore the key // code ban dau
    //return SortedRemove(&(currentThread->priority));  // code duoc sua
}
...
List::SortedInsert(void *item, int sortKey)
{
    ...
    if (IsEmpty()) {	// if list is empty, put
        first = element;
        last = element;
    } else if (sortKey < first->key) { // sua < thanh <=
		// item goes on front of list
	element->next = first;
	first = element;
    } else {		// look for first elt in list bigger than item
        for (ptr = first; ptr->next != NULL; ptr = ptr->next) {
            if (sortKey < ptr->next->key) { // sua < thanh <=
