...
OpenFile* FileSystem::Open(char*name,int type)
{
    ...
    Directory *Dire = new Directory(NumDirEntries);
    OpenFile *oFile = NULL;
    int Sector;
    DEBUG('f', "Opening file %s\n", name);
    Dire->FetchFrom(directoryFile);
    Sector = Dire->Find(name);
    if (Sector >= 0)
        openf[Slot] = new OpenFile(Sector, type);   // name was found in directory
    delete Dire;
    //index++;
    return openf[Slot];             // return NULL if not found
}
